...
namespace ResourceManager
{
	...
	void ResourceManager::Update()
	{
		...
		{
			std::string holder = "Resources/Texture/" + m_texturePath.at(i);
		...
		{
			std::string holder = "Resources/Font/" + m_fontPath.at(i);
		...
		{
			std::string holder = "Resources/Music/" + m_musicPath.at(i);
		...
		{
			std::string holder = "Resources/Sound/" + m_soundPath.at(i);
		...
		{
			std::string holder = "Resources/Map/" + m_mapPath.at(i);
		...
		{
			std::string holder = "Resources/Config/" + m_configPath.at(i);
	...
	{
		std::string holder = "Resources/Texture/" + location;
		SDL_Surface * temp;
		const char * tempLocation = holder.c_str();
		if (!(temp = IMG_Load(tempLocation)))
		{
			std::cout << "Could not load " << id << "! Loading Placeholder!" << std::endl;
			temp = IMG_Load("Resources/Texture/placeholder.png");
			m_textureSize.push_back(GetFileSize("Resources/Texture/placeholder.png"));
	...
	{
		std::string temp = "Resources/Font/" + location;
	...
	{
		std::string holder = "Resources/Music/" + location;
		Mix_Music * temp;
		const char * tempLocation = holder.c_str();
		if (!(temp = Mix_LoadMUS(tempLocation)))
		{
			std::cout << "Could not load " << id << "! Loading Placeholder!" << std::endl;
			temp = Mix_LoadMUS("Resources/Music/placeholder.wav");
			m_music.push_back(temp);
			m_musicKey.push_back(id);
			m_musicPath.push_back("placeholder.wav");
			m_musicSize.push_back(GetFileSize("Resources/Music/placeholder.wav"));
		}
		else
		{
	...
	{
		std::string holder = "Resources/Sound/" + location;
		Mix_Chunk * temp;
		const char * tempLocation = holder.c_str();
		if (!(temp = Mix_LoadWAV(tempLocation)))
		{
			std::cout << "Could not load " << id << "! Loading Placeholder!" << std::endl;
			temp = Mix_LoadWAV("Resources/Sound/placeholder.wav");
			m_sound.push_back(temp);
			m_soundKey.push_back(id);
			m_soundPath.push_back("placeholder.wav");
			m_soundSize.push_back(GetFileSize("Resources/Sound/placeholder.wav"));
		}
		else
		{
	...
	{
		std::string holder = "Resources/Map/" + location;
		const char * tempLocation = holder.c_str();
		JSONReader temp = JSONReader(tempLocation);
		if ((temp.load(tempLocation) != 1))
		{
			std::cout << "Could not load " << id << "! Loading Placeholder!" << std::endl;
			temp = JSONReader("Resources/Map/placeholder.json");
			temp.load("Resources/Map/placeholder.json");
			m_map.push_back(temp.loadLevel());
			m_mapKey.push_back(id);
			m_mapPath.push_back("placeholder.json");
			m_mapSize.push_back(GetFileSize("Resources/Map/placeholder.json"));
		}
		else
		{
	...
	{
		std::string holder = "Resources/Config/" + location;
		const char * tempLocation = holder.c_str();
		...
		if ((temp->load(tempLocation) != 1))
		{
			...
			m_soundSize.push_back(GetFileSize("Resources/config/placeholder.json"));
		}
		else
		{
	...
	void ResourceManager::AddConfigElement(std::string id, std::string configElementId, std::string configElementLoc)
	{
		...
		std::string holder = "Resources/Config/" + m_configPath.at(idholder);
		const char * tempLocation = holder.c_str();
		JSONReader *temp = new JSONReader(tempLocation);
